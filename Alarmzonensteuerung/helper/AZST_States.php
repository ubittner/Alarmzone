...
trait AZST_States
{
    ...
    public function UpdateStates(): bool
    {
        ...
        $update13 = $this->UpdateInternalAlarmSiren();
        $update14 = $this->UpdateAlarmLight();
        $update15 = $this->UpdateAlarmCall();
        $update16 = $this->UpdatePanicAlarm();
        if (!$update1 ||
            !$update2 ||
            !$update3 ||
            !$update4 ||
    ...
    /**
     * Updates the state of the internal alarm siren.
     *
     * @return bool
     * false =  an error occurred,
     * true =   successful
     ...
     * @throws Exception
     */
    public function UpdateInternalAlarmSiren(): bool
    {
        $this->SendDebug(__FUNCTION__, 'wird ausgeführt', 0);
        if ($this->CheckMaintenance()) {
            return false;
        ...
        $this->SetValue('InternalAlarmSiren', $state);
        return $result;
    }
